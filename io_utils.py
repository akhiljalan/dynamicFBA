...
def plot_results_from_csv_one_by_one(filepath: str, figsize=(8, 4)) -> None:
    """
    Load a CSV generated by save_results_to_csv() and show one plot at a time
    for biomass and each metabolite vs time. User advances by closing each window.
    """
    df = pd.read_csv(filepath)
    ...
    for comp in components:
        ...
        plt.plot(time, df[comp], linewidth=2, marker='o')
        plt.title(comp)
        plt.xlabel("Time (hr)")
        plt.ylabel("Concentration (mM)" if comp != 'biomass_gDW' else "Biomass (gDW)")
        plt.grid(True)
        plt.tight_layout()
        plt.show()  # This blocks until the figure window is closed
